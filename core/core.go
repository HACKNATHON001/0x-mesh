...
func (app *App) GetOrders(page, perPage int, snapshotID string) (*rpc.GetOrdersResponse, error) {
	...
	if snapshotID == "" {
		...
		expirationTimeSeconds := time.Now().Unix() + 60 // 1min from now
		app.snapshotExpirationWatcher.Add(expirationTimeSeconds, snapshotID)
		app.muIdToSnapshotInfo.Lock()
		...
		app.snapshotExpirationWatcher.Remove(info.ExpirationTimeSeconds, snapshotID)
		expirationTimeSeconds := time.Now().Unix() + 60 // 1min from now
		app.snapshotExpirationWatcher.Add(expirationTimeSeconds, snapshotID)
		app.idToSnapshotInfo[snapshotID] = snapshotInfo{
			Snapshot:              snapshot,
