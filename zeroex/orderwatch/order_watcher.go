...
func (w *Watcher) setupInMemoryOrderState(signedOrder *zeroex.SignedOrder) error {
	...
	w.expirationWatcher.Add(signedOrder.ExpirationTimeSeconds.Int64(), orderHash.Hex())
...
func (w *Watcher) rewatchOrder(order *meshdb.Order, orderInfo *zeroex.AcceptedOrderInfo) {
	...
	w.expirationWatcher.Add(order.SignedOrder.ExpirationTimeSeconds.Int64(), order.Hash.Hex())
}
...
func (w *Watcher) unwatchOrder(order *meshdb.Order) {
	...
	w.expirationWatcher.Remove(order.SignedOrder.ExpirationTimeSeconds.Int64(), order.Hash.Hex())
}
